    ...
    for row in range(len(symbol_grid)):
        for col in range(len(symbol_grid[0])):
            ...
            if val in list(props.keys()):
                ...
                grid[row][col] = combine_concat_symbols_into_new_props(
                    val, props)
...
def combine_concat_symbols_into_new_props(val, props):
    sum = 0
    for letter in val:
...
def get_node_edge_labels(node, labels):
    ...
    for label_key in labels.keys():
        ...
        if len(label_key_list) > 1 and label_key_list[1] == node:
...
def create_ts(map_path="maps/alphabetical_maps/map_multiple_alpha_symbols_complex.csv", init_node='a', prune=True, display=True):
    ...
    reduced_symbols = list(set(i for j in symbol_grid for i in j))
    props = assign_props(reduced_symbols)
    ...
    return create_ts_from_edges(edges, list(unique_clusters.keys()), props, init_node=init_node, prune=prune, display=display)


def create_ts_from_edges(edges, unique_clusters_labels, props, init_node='a', prune=True, display=True):
    # print(f"Edges: {edges}")
    ...
    labels = create_transitions(
        intermediate_G, edges, unique_clusters_labels, spl)
    labels = prune_labels(unique_clusters_labels,
...
if __name__ == '__main__':
    ...
    ts, _ = create_ts(
        'lomap/lomap/tests/maps/unit_test_maps/alphabetical_maps/example11.csv', prune=True)
