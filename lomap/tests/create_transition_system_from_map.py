...
def load_symbol_map(file_path, start_and_goal=True):
    grid = []
    start = [0, 0]
    goal = [0, 0]
    # Load from the file
    with open(file_path, 'r') as map_file:
        ...
        for i, row in enumerate(reader):
            ...
            if start_and_goal and i == 0:
                start[0] = int(row[1])
                start[1] = int(row[2])
            elif start_and_goal and i == 1:
                goal[0] = int(row[1])
                goal[1] = int(row[2])
            # load the map
            else:
...
def assign_props(grid):
    ...
    single_chars = [x for x in reduced if (
        len(x) == 1 and x != EMPTY_SYMBOL_NUMERIC and x != WALL_SYMBOL)]
    multi_chars = [x for x in reduced if (
        len(x) > 1 and x != EMPTY_SYMBOL_NUMERIC and x != WALL_SYMBOL)]
...
def prune_labels(nodes, edges, labels, spl, prune):

    # for these purposes, need to remove every sub label e.g. 1.1 1.2 and replace it with 1, since same symbol in map
    for node in nodes:
        for label_key in labels.keys():
            pruned_label = list(set(map(lambda x: str(math.floor(float(x))), labels.get(
                label_key))))  # set to remove duplicates
    ...
    if prune:
        ...
        case_1(nodes, edges, pruned_labels)
        ...
        for node in nodes:
            ...
            case_2(node, node_outgoing_labels, spl)
            case_3(simplfied_node_rep, node_outgoing_labels)
            # case_4(node_outgoing_labels)
...
'''
CASES:
1) if nodes and transitions are the same, combine them
2) if any outgoing edges share transition label, only keep label on the closest edge. If edges all the same distance, remove from all.
3) if there is an outgoing edge containing same label as node remove it 
4) remove empty symbol from transition
...
def case_1(nodes, edges, pruned_labels):
    ...
    for i in range(len(nodes)):
        ...
        if node not in to_remove:
            node_incoming_labels, node_outgoing_labels = get_node_edge_labels(
                node, pruned_labels)
            for other_node in nodes[i+1:]:
                if other_node not in to_remove:
                    # reduced node labels are the same
                    if int(float(node)) == int(float(other_node)):
                        other_node_incoming_labels, other_node_outgoing_labels = get_node_edge_labels(
                            other_node, pruned_labels)
    ...
    for reduced_node in to_remove.keys():
        ...
        for node_to_remove in sorted_equivalent_nodes[1:]:
            ...
            pruned_labels_updated = {k: list(filter(lambda x: node_to_remove not in x, v))
                                     for k, v in pruned_labels.items() if node_to_remove not in k}
            pruned_labels.clear()
            # to keep same object for cleanliness
            pruned_labels.update(pruned_labels_updated)
...
def case_2(node, node_outgoing_labels, spl):
    # and node != EMPTY_SYMBOL_NUMERIC: #if there is more than one outgoing edge
    if len(list(node_outgoing_labels.keys())) > 1:

        # if item duplicated in list of all edge labels concanated then it occurs on at least more than one edge
        list_of_all_edge_values = [elem for sublist in list(
            node_outgoing_labels.values()) for elem in sublist]
        node_outgoing_label_intersections = [
            k for k, v in Counter(list_of_all_edge_values).items() if v > 1]
        ...
        # if more than one reduced (eg. 1.1, 1.0 = 1) label on different outgoing edges match
        if len(node_outgoing_label_intersections) > 0:
            ...
            for shared in node_outgoing_label_intersections:
                            ...
                            if int(float(key)) == int(shared):
                                if connected_node in distance_to_shared:
                                    if connected_nodes_connections_path_length[key] < distance_to_shared[connected_node]:
                                        # update
                                        distance_to_shared[connected_node] = connected_nodes_connections_path_length[key]
                                else:
                                    # init
                                    distance_to_shared[connected_node] = connected_nodes_connections_path_length[key]
                ...
                nodes_with_shortest_path_to_shared = [
                    key for key in distance_to_shared if distance_to_shared[key] == min(distance_to_shared.values())]
...
def case_3(simplfied_node_rep, node_outgoing_labels):
    for key in node_outgoing_labels.keys():
...
def case_4(labels):
    for key in labels.keys():
        empty_symbols = [s for s in labels.get(
            key) if EMPTY_SYMBOL_NUMERIC == s]
        for s in empty_symbols:
            labels.get(key).remove(s)


def case_5(labels):
    node_outgoing_labels_updated = {k: v for k, v in labels.items() if v != []}
    labels.clear()
    labels.update(
        node_outgoing_labels_updated)  # to keep same object for cleanliness\


def empty_cleanup(pruned_labels):
    case_4(pruned_labels)
...
def create_ts(map_path="maps/alphabetical_maps/map_multiple_alpha_symbols_complex.csv", init_node=None, prune=True, display=True, start_and_goal_in_map=True):
    '''
    ...
    intermediate_G, edges, unique_clusters, props = create_initial_ts_from_map_clusters(map_path, start_and_goal_in_map)

    spl = dict(nx.all_pairs_shortest_path_length(intermediate_G))

    labels = create_transitions(
        intermediate_G, edges, list(unique_clusters.keys()), spl)
    labels = prune_labels(list(unique_clusters.keys()),
                          edges, labels, spl, prune)

    G = nx.DiGraph()

    alphabetical_edges, label_mapping = convert_edges_and_add_labels_alphabetical(
        labels, props, edges)
...
def create_initial_ts_from_map_clusters(map_path, start_and_goal_in_map):
    symbol_grid, start, goal = load_symbol_map(map_path, start_and_goal=start_and_goal_in_map)
    ...
    return intermediate_G, edges, unique_clusters, props
...
class TestTSCreation(unittest.TestCase):
    ...
    def test_example_1(self):
        nodes = ['0', '1.1', '1.0', '2', '4']
        out_edges = [('0', '1.0'), ('0', '1.1'), ('1.0', '2'), ('1.0', '0'),
                     ('1.1', '4'), ('1.1', '0'), ('2', '1.0'), ('4', '1.1')]
        in_edges = [('1.1', '0'), ('1.0', '0'), ('0', '1.0'), ('2', '1.0'),
                    ('0', '1.1'), ('4', '1.1'), ('1.0', '2'), ('1.1', '4')]
        adj = {'0': {'1.0': {'pi': ['b'], 'weight': 0}, '1.1': {'pi': ['c'], 'weight': 0}}, '1.0': {'2': {'pi': ['b'], 'weight': 0}, '0': {'pi': ['c'], 'weight': 0}}, '1.1': {
            '4': {'pi': ['c'], 'weight': 0}, '0': {'pi': ['b'], 'weight': 0}}, '2': {'1.0': {'pi': ['c', 'a'], 'weight': 0}}, '4': {'1.1': {'pi': ['b', 'a'], 'weight': 0}}}
    ...
    def test_example_2(self):
        nodes = ['0.0', '1', '4', '2', '0.1']
        out_edges = [('0.0', '1'), ('0.0', '4'), ('0.0', '2'), ('1', '0.0'),
                     ('4', '0.0'), ('2', '0.0'), ('0.1', '1'), ('0.1', '2')]
        in_edges = [('2', '0.0'), ('4', '0.0'), ('1', '0.0'), ('0.0', '1'),
                    ('0.1', '1'), ('0.0', '4'), ('0.0', '2'), ('0.1', '2')]
        adj = {'0.0': {'1': {'pi': ['a'], 'weight': 0}, '4': {'pi': ['c'], 'weight': 0}, '2': {'pi': ['b'], 'weight': 0}}, '1': {'0.0': {'pi': ['c'], 'weight': 0}}, '4': {
            '0.0': {'pi': ['a', 'b'], 'weight': 0}}, '2': {'0.0': {'pi': ['c'], 'weight': 0}}, '0.1': {'1': {'pi': ['a'], 'weight': 0}, '2': {'pi': ['b'], 'weight': 0}}}
    ...
    def test_example_4(self):
        ...
        out_edges = [('4.0', '1.0'), ('1.0', '2.0'), ('1.0', '4.0'), ('4.1', '2.1'), ('2.1',
                                                                                      '1.1'), ('2.1', '4.1'), ('2.0', '1.0'), ('1.1', '2.1'), ('0', '4.1'), ('0', '4.0')]
        in_edges = [('1.0', '4.0'), ('0', '4.0'), ('4.0', '1.0'), ('2.0', '1.0'), ('2.1', '4.1'),
                    ('0', '4.1'), ('4.1', '2.1'), ('1.1', '2.1'), ('1.0', '2.0'), ('2.1', '1.1')]
        adj = {'4.0': {'1.0': {'pi': ['b', 'a'], 'weight': 0}}, '1.0': {'2.0': {'pi': ['b'], 'weight': 0}, '4.0': {'pi': ['c'], 'weight': 0}}, '4.1': {'2.1': {'pi': ['b', 'a'], 'weight': 0}}, '2.1': {'1.1': {'pi': [
            'a'], 'weight': 0}, '4.1': {'pi': ['c'], 'weight': 0}}, '2.0': {'1.0': {'pi': ['c', 'a'], 'weight': 0}}, '1.1': {'2.1': {'pi': ['c', 'b'], 'weight': 0}}, '0': {'4.1': {'pi': ['b'], 'weight': 0}, '4.0': {'pi': ['a'], 'weight': 0}}}
    ...
    def test_example_5(self):
        ...
        out_edges = [('4.0', '8'), ('4.0', '2.1'), ('8', '4.1'), ('2.1', '4.0'), ('4.1', '8'), ('4.1', '1.0'), ('1.0', '2.0'),
                     ('1.0', '4.1'), ('4.2', '8'), ('4.2', '2.2'), ('2.2', '1.1'), ('2.2', '4.2'), ('2.0', '1.0'), ('1.1', '2.2')]
        in_edges = [('2.1', '4.0'), ('4.0', '8'), ('4.1', '8'), ('4.2', '8'), ('4.0', '2.1'), ('1.0', '4.1'), ('8', '4.1'),
                    ('4.1', '1.0'), ('2.0', '1.0'), ('2.2', '4.2'), ('4.2', '2.2'), ('1.1', '2.2'), ('1.0', '2.0'), ('2.2', '1.1')]
        adj = {'4.0': {'8': {'pi': ['d', 'a'], 'weight': 0}, '2.1': {'pi': ['b'], 'weight': 0}}, '8': {'4.1': {'pi': ['a'], 'weight': 0}}, '2.1': {'4.0': {'pi': ['d', 'c', 'a'], 'weight': 0}}, '4.1': {'8': {'pi': ['d'], 'weight': 0}, '1.0': {'pi': ['b', 'a'], 'weight': 0}}, '1.0': {'2.0': {'pi': ['b'], 'weight': 0}, '4.1': {
            'pi': ['d', 'c'], 'weight': 0}}, '4.2': {'8': {'pi': ['d'], 'weight': 0}, '2.2': {'pi': ['b', 'a'], 'weight': 0}}, '2.2': {'1.1': {'pi': ['a'], 'weight': 0}, '4.2': {'pi': ['d', 'c'], 'weight': 0}}, '2.0': {'1.0': {'pi': ['d', 'c', 'a'], 'weight': 0}}, '1.1': {'2.2': {'pi': ['d', 'c', 'b'], 'weight': 0}}}
...
if __name__ == '__main__':
    ...
    ts, _ = create_ts(
        'maps/unit_test_maps/alphabetical_maps/example7.csv', prune=True)
