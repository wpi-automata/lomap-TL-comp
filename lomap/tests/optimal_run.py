...
def min_bottleneck_cycle_serial(g, s, f):
    """ Serial version of min_bottleneck_cycle that doesn't require pp.
    
    Returns the minimum bottleneck cycle from s to f in graph g.
    This is a non-parallel implementation of the algorithm.
    """
    import itertools
    
    logger.info('Computing S->S+F paths (serial)')
    ...
    logger.info('Computing F->S paths (serial)')
    d_f_to_s = subset_to_subset_dijkstra_path_value(f, g, s, degen_paths=True)
    
    # Compute shortest S-bottleneck paths between vertices in s
    logger.info('Computing S-bottleneck paths (serial)')
    d_bot = subset_to_subset_dijkstra_path_value(s, g_s, s, combine_fn='max', degen_paths=False)
    
    # Find the triple in F x S x S that minimizes C(f,s1,s2)
    logger.info('Finding best cycle (serial)')
    cost_star = float('inf')
    len_star = float('inf')
    cycle_star = None
...
def min_bottleneck_cycle(g, s, f):
    ...
    global pp_installed
    if not pp_installed:
        logger.info('Parallel Python not installed, using serial version')
        return min_bottleneck_cycle_serial(g, s, f)
