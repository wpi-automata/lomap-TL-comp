...
def  weight_env_and_buchi_product(ltl_spec, json_file_path, map_path, start_state, visualize=True): #TODO: Clean this up to look like a method and not main
    ...
    # Helper function to check if a node is an accepting state
    def is_accepting_state(node):
        """Check if a node is an accepting state by looking for 'accept_all' in node attributes or label"""
        if('accept' in node[1]):
            return True
        
        # Also check the label string for 'accept_all' 
        # Labels are in format like "({'rectangular_table'}, 'accept_all')"
        node_label = labels.get(node, '')
        if node_label:
            # Check if 'accept_all' appears in the label string
            if "'accept_all'" in str(node_label) or '"accept_all"' in str(node_label) or 'accept_all' in str(node_label):
                return True
